...
public class JSONSetCIPMsg extends JSONMsg {
	...
	public JSONSetCIPMsg(ArrayList<String> cntrIP, OVSDBImpl ovsdbImpl, int msgId) throws OVSDBBridgeUnknown {
		this.ciplist = cntrIP;
		this.ovs = ovsdbImpl;
		this.id = msgId;
		this.setcipstr = null;
	...
	private void buildSetCntlIPString() throws OVSDBBridgeUnknown {
		setcipstr = "{\"method\":\"transact\",\"id\":" + id + ",\"params\":" + "[\"Open_vSwitch\",";
	...
	private void buildRowsExistingCIPs() {
		Iterator<Entry<String, OVSController>> iter = ovs.controller.entrySet().iterator();
	...
	private void buildRowsController() throws OVSDBBridgeUnknown {
		...
		if (existingCIP.size() > 1) {
			...
			setcipstr += "{\"rows\":[{\"controller\":[" + "\"uuid\",\""+ existingCIP.get(0) +"\"]}]";
		}
		
		bridgeuuid = getOvsbr0Bridgeuuid();
		if (bridgeuuid == null) {
			throw new RuntimeException("ovs.bridge.get("+ bridgeuuid + ")" + " returned Null ub set-controller-ip message");
		}
		setcipstr += ",\"until\":\"==\",\"where\":[[\"_uuid\",\"==\"," + "[\"uuid\",\""+ bridgeuuid +"\"]]]," + "\"timeout\":0,\"op\":\"wait\",\"table\":\"Bridge\",\"columns\":" + "[\"controller\"]},";
	}
	...
	private void buildRowsBridge() {
		...
		if (ovs.bridge.size() == 1) {
			...
			setcipstr += "\"until\":\"==\",\"where\":[[\"_uuid\",\"==\"," +
					"[\"uuid\""	+ ",\""+ dbuuid +"\"]]],\"timeout\":0,\"op\":\"wait\",\"table" +
					"\":\"Open_vSwitch\",\"columns\":[\"bridges\"]},";
		}
	}
	...
	private void buildInserts() {
		...
		for (int i=0; i<ciplist.size(); i++) {
			...
			setcipstr += "{\"uuid-name\":\""+ fakerow +"\",\"op\":\"insert\"" + ",\"table\":\"Controller\",\"row\":{\"target\":" + "\""+ ciplist.get(i) +"\"}},";
			fakerowlist.add(fakerow);
		}
		
		// insert where
		setcipstr += "{\"where\":[[\"_uuid\",\"==\",[\"uuid\",\""+ bridgeuuid + "\"]]],\"op\":\"update\",\"table\":\"Bridge\"," + "\"row\":{\"controller\":";
	...
	private void buildComment() {
		setcipstr += "{\"comment\":\"ovs-vsctl: ovs-vsctl --no-wait " +
				"set-controller br-tun tcp:192.168." +
				"200.199\",\"op\":\"comment\"}]}";
	} //br-tun -> ovs-br0
	
	private String getOvsbr0Bridgeuuid() throws OVSDBBridgeUnknown {
		Iterator<Entry<String, OVSBridge>> iter = ovs.bridge.entrySet().iterator();
