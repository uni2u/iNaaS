...
public class JSONDelPortMsg extends JSONMsg {
	...
	*/
	public JSONDelPortMsg(String name, String portHash, OVSDBImpl tsw, int messageId) throws OVSDBBridgeUnknown {
		this.name = name;
	...
	private void buildDelPortMsg() throws OVSDBBridgeUnknown {
		completeString = "{\"method\":\"transact\",\"id\":"+id+",\"params\":["+" \"Open_vSwitch\", ";
	...
	private String rowsUntil() {
		...
		String mid = "\"]}],\"until\":\"==\",\"where\":[[\"_uuid\",\"==\"" + ",[\"uuid\",\"";
		String end = "\"]]],\"timeout\":0,\"op\":\"wait\",\"table\":\"Port\"" + ",\"columns\":[\"interfaces\"]},";
		...
		if (numbridges > 1) {
			...
			String mid2 = "\"],[\"uuid\",\"";
			String mid3 = "\"]]]}],\"until\":\"==\",\"where\":[[\"_uuid\"," + "\"==\",[\"uuid\",\"";
			end = "\"]]],\"timeout\":0,\"op\":\"wait\",\"table\":\"Open_" + "vSwitch\"" + ",\"columns\":[\"bridges\"]},";
			Iterator<Entry<String, OVSDatabase>> iter1 = tsw.open_vswitch.entrySet().iterator();
			rows += start;
			...
			while(iter1.hasNext()) {
				Entry<String, OVSDatabase> e1 = iter1.next();
				dbuuid = e1.getKey();
				String bridgeuuid = (String) e1.getValue().getNew().getBridges().get(1);
				rows += start + bridgeuuid + mid + dbuuid + end;
				break; // just one database
			}
		}
		
		Iterator<Entry<String, OVSBridge>> iter5 = tsw.bridge.entrySet().iterator();
		...
		while (iter5.hasNext()) {
			...
			end = "\"]]],\"timeout\":0,\"op\":\"wait\",\"table\":\"Bridge\"" + ",\"columns\":[\"ports\"]},";
	...
	private String update() throws OVSDBBridgeUnknown {
		...
		String start1 = "{\"where\":[[\"_uuid\",\"==\",[\"uuid\",\"" + bridgeuuid + "\"]]],\"op\":\"update\",\"table\":\"Bridge\"," + "\"row\":{\"ports\":[\"set\",[";
		String ports = "";
		String porthdr = "[\"uuid\",\"";
		String porttlr = "\"]";
		...
		if ( br == null ) {
			throw new RuntimeException("tsw.bridge.get("+ bridgeuuid + ") returned Null");
		}
	...
	private String getBridgeuuid() throws OVSDBBridgeUnknown {
		...
		Iterator<Entry<String, OVSBridge>> iter = tsw.bridge.entrySet().iterator();
	...
	public void writeTo(ChannelBuffer buf) {
		if (Main.debug) {
			Logger.debug("sent del-port message to:" + name + " msg-id:"+ id + " @sw: {} ", HexString.toHexString(tsw.getDpid()));
		}
